...
(async function initAntigravityBackground() {
  ...
  function createPointsData() {
    ...
    function addSample(x, y) {
      ...
      active.push(sample);
      grid[
        gridIndex(Math.floor(x / cellSize), Math.floor(y / cellSize))
      ] = samples.length - 1;
    }
  ...
  function createPositionTexture(pointsData, length) {
  ...
  const posTex = createPositionTexture(pointsData.slice(0, count * 2), length);
  ...
  function updateRingPosition() {
    const idleX =
      ((noise1D(elapsedTime * 0.66 + 94.234, idleSeedX) - 0.5) * 2) *
      0.2;
    const idleY =
      ((noise1D(elapsedTime * 0.75 + 21.028, idleSeedY) - 0.5) * 2) *
      0.1;
    if (isIntersecting) {
      cursorPos.set(
  ...
  function renderFrame() {
    ...
    const ringRadius = 0.175 + Math.sin(elapsedTime * 1.0) * 0.03 +
      Math.cos(elapsedTime * 3.0) * 0.02;
    const particleScale = renderer.domElement.width / pixelRatio() / 2000 *
      CONFIG.particlesScale;
  ...
  window.addEventListener("beforeunload", () => {
    ...
    simMaterial.dispose();
    simScene.children[0].geometry.dispose();
    posTex.dispose();
    rt1.dispose();
    rt2.dispose();
...
function createUploadFillController(layer) {
  if (!layer) {
    return {
      start() {},
      complete() {
  ...
  function buildClipPath(now) {
    ...
    return `polygon(0 100%, 0 ${points[0].split(" ")[1]}, ${
      points.join(", ")
    }, 100% 100%)`;
  }
  ...
  function tick(now) {
    ...
    if (state === "filling") {
      ...
      const primingProgress = easeInOutSine(
        clamp(elapsed / PRIMING_MS, 0, 1),
      ) * PRIMING_PROGRESS;
    ...
    if (state === "finishing") {
      ...
      progress = finishFromProgress +
        ((1 - finishFromProgress) * easeInOutSine(t));
      if (t >= 1) {
        state = "fading";
        fadeStartAt = now;
      }
...
const uploadFill = createUploadFillController(uploadFillLayer);
...
function renderFileList(files) {
  ...
  for (const f of files) {
    ...
    item.innerHTML =
      `<span class="file-icon">📄</span><span class="file-name">${f.name}</span><span class="file-size">${
        formatSize(f.size)
      }</span>`;
...
async function upload(body, name) {
  ...
  msg.innerHTML =
    `<div class="session-key" id="sessionKey" title="클릭해서 복사">${key}</div>`;
  ...
  try {
    uploadResp = await uploadWithProgress(
      `/upload?key=${key}&fileName=${encodeURIComponent(name)}${
        isTxt ? "&isTxt" : ""
      }`,
      body,
      (fraction) => {
